...
template <typename T>
concept DurationConcept = std::same_as<T, std::chrono::nanoseconds> || std::same_as<T, std::chrono::microseconds>
...
struct Task {
    Task(const std::function<void()>& func, const DurationType& period, const std::size_t& id) :
        fun(func), period(period), uniqueId(id) { }
...
public:
    ...
    void removeTask(std::size_t uniqueId) noexcept {
        std::lock_guard lg(vecMutex_);
        vec_.erase(
            std::remove_if(vec_.begin(), vec_.end(), [&](const auto& task) { return task.uniqueId == uniqueId; }));
    }
